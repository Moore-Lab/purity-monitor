...
def run_rga(rga, mass1, mass2, steps):
    ...
    '''
    rga.turn_on_filament()
    mass, pressure, total = rga.read_spectrum(mass1, mass2, steps) # read analog scan of mass range from (default) 1-200 amu with max resolution of 10 steps per amu
...
def date_range(start, end):
    '''Getting a list of the range of dates given the start and end dates
    '''
    ...
    # generate a list of consecutive dates and convert it back to strings
    date_list = [start_date + datetime.timedelta(days=x) for x in range((end_date - start_date).days + 1)]
    date_list = [date.strftime(date_format)+'.h5' for date in date_list]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path',
                        ...
                        type=str,
                        default='C:/Data/{}'.format(datetime.datetime.now().strftime('%Y%m%d')))
    ...
    while rga_data:
        try:
            ...
            store = pd.HDFStore(args.file_path+'/RGAScan.h5')
    ...
    # turning off the filament of the RGA after taking measurements
    RGA.turn_off_filament()

    # copying the temperature log files to the same directory as the rga scans

    # providing the new and old directory paths
    origin = 'C:/Data/StandStatus/'
    target = args.file_path

    slowcontrols = os.listdir(origin)

    # get the original and current date
    date_0 = os.path.basename(args.file_path).rstrip('/')
    date_f = datetime.datetime.now().strftime('%Y%m%d')
    
    files = date_range(date_0, date_f)
